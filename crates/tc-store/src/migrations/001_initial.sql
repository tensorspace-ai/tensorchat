-- Migration 001 — the version 1 baseline.
--
-- FROZEN. This file records what version 1 looked like, so a database created
-- by an older build has a known starting point to be upgraded from. Never edit
-- it: changing history would make an upgraded database diverge from a fresh
-- one. New schema goes in a new numbered migration *and* in `schema.sql`, which
-- is what a fresh database gets; `migrations_match_a_fresh_schema` asserts the
-- two paths agree.
--
-- Conventions:
